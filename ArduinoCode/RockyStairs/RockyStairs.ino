...
void loop()
{
  ...
  if((cur_time - prev_time) > UPDATE_TIME_MS)
  {
    ...
    if(start_flag)
    {
      if (cur_time - startTime > 3000)
      {
        if (beepFlag == 1)
        {
          beepFlag = 0;
          buzzer.playFrequency(DIV_BY_10 | 600, 1000, 15);
          startTime = cur_time;
        }
        angle_rad = ((float)angle)/1000/180*3.14159 - FIXED_ANGLE_CORRECTION - del_theta;
      }
      else if (cur_time - startTime > 1000)
      {
        if (beepFlag == 0)
        {
          beepFlag = 1;
          buzzer.playFrequency(DIV_BY_10 | 200, 1000, 15);
        }
        angle_rad = ((float)angle)/1000/180*3.14159 - 0.30 - del_theta;
      }
      else
      {
        angle_rad = ((float)angle)/1000/180*3.14159 - FIXED_ANGLE_CORRECTION - del_theta;
